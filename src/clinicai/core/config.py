...
class IntakeSettings(BaseSettings):
    ...
    max_questions: int = Field(default=12, description="Maximum intake questions allowed (default: 12)")
...
class LLMInteractionSettings(BaseSettings):
    ...
    batch_size: int = Field(default=10, description="Batch size for batch mode (default: 10)")
...
class Settings(BaseSettings):
    ...
    def __init__(self, **kwargs):
        ...
        # Try to load secrets from Azure Key Vault if available (can be disabled)
        # Set DISABLE_KEY_VAULT=true to force env vars only (useful for local/dev)
        key_vault_disabled = os.getenv("DISABLE_KEY_VAULT", "").lower() == "true"
        key_vault_secrets = {}
        if not key_vault_disabled:
            try:
                ...
                logger = logging.getLogger("clinicai")
                # Only log at debug level - this is expected in local development
