    ...
    for version_to_try in api_versions_to_try:
        try:
            ...
            if version_to_try != api_version:
                return (
                    True,
                    f"Deployment validated successfully with API version '{version_to_try}' (original was '{api_version}'). Consider updating AZURE_OPENAI_API_VERSION to '{version_to_try}'.",
                )
            return True, None
