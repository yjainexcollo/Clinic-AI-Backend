...
def extract_text_with_quality(image_path: str) -> OCRResult:
    ...
    try:
        ...
        medication_keywords = [
            'mg', 'ml', 'tablet', 'capsule', 'syrup', 'drops', 'injection',
            'twice', 'daily', 'morning', 'evening', 'night', 'before', 'after',
            'food', 'empty stomach', 'with water', 'prescription', 'dosage',
            'medicine', 'drug', 'pharmaceutical', 'rx', 'take', 'apply'
        ]
        ...
        quality, suggestions = _assess_ocr_quality(
            normalized_text, avg_confidence, word_count, 
            has_medication_keywords, len(extracted_medications)
        )
...
def _extract_medication_names(text: str) -> List[str]:
    ...
    exclude_words = {
        ...
        'take', 'with', 'food', 'water', 'morning', 'evening', 'night',
        'daily', 'twice', 'once', 'before', 'after', 'empty', 'stomach'
    }
    
    # Filter out short words and common words
    filtered_medications = [
        med for med in medications 
        if len(med) > 3 and med.lower() not in exclude_words
    ]
    
    return list(set(filtered_medications))  # Remove duplicates


def _assess_ocr_quality(text: str, confidence: float, word_count: int, 
                       has_medication_keywords: bool, medication_count: int) -> tuple[str, List[str]]:
