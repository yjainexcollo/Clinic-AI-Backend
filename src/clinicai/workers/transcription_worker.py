...
class TranscriptionWorker:
    ...
    async def get_audio_data(self, audio_file_id: str) -> Optional[bytes]:
        ...
        try:
            ...
            # Get blob reference
            from clinicai.adapters.db.mongo.repositories.blob_file_repository import (
                BlobFileRepository,
            )
    ...
    async def process_job(self, job_data: dict, message_id: str, pop_receipt: str):
        ...
        try:
            ...
            logger.error(
                f"Error during idempotency check/claim: {idempotency_check_error}",
                exc_info=True,
            )
            ...
            try:
                ...
                while db_save_attempts < max_db_save_attempts and not db_save_success:
                    try:
                        ...
                        if (
                            saved_visit
                            and saved_visit.transcription_session
                            ...
                            logger.warning(
                                f"⚠️  Transcript not found in DB after use case, attempting save (attempt {db_save_attempts + 1}/{max_db_save_attempts})"
                            )
            ...
            if is_permanent_error:
                ...
                if db_save_success:
                    try:
                        ...
                        logger.error(
                            f"Failed to delete message after permanent error: {delete_error}",
                            exc_info=True,
                        )
                else:
                    logger.error(
