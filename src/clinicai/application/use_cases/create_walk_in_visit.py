...
class CreateWalkInVisitRequest:
    """Request for creating a walk-in visit. Uses first_name and last_name (no separate name)."""
...
class CreateWalkInVisitUseCase:
    ...
    async def execute(self, request: CreateWalkInVisitRequest, doctor_id: str) -> CreateWalkInVisitResponse:
        ...
        existing_patient = await self._patient_repository.find_by_name_and_mobile(full_name, request.mobile, doctor_id)
