        ...
        try:
            ...
            for scenario, docs in docs_by_scenario.items():
                if len(docs) > 1:
                    ...
                    docs.sort(
                        key=lambda d: (len(getattr(d, "versions", [])), str(d.id)),
                        reverse=True,
                    )
                    ...
                    for duplicate_doc in docs[1:]:
                        ...
                        logger.info(
                            f"🗑️  Marking duplicate document for deletion: scenario '{scenario}', ID: {duplicate_doc.id}"
                        )
            ...
            for doc in invalid_docs:
                try:
                    await doc.delete()
                    logger.info(
                        f"✅ Deleted invalid/duplicate document ID: {doc.id}, scenario: {getattr(doc, 'scenario', 'unknown')}"
                    )
                except Exception as e:
