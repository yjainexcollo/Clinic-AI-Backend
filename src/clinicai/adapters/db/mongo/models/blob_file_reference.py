...
class BlobFileReference(Document):
    ...
    category: str = Field(
        default="general", description="File category (adhoc, visit, medication, etc.)"
    )
    ...
    expires_at: Optional[datetime] = Field(
        None, description="File expiry date (for temporary files)"
    )

    # Status
    is_active: bool = Field(default=True, description="Whether the file is active")
    is_deleted: bool = Field(
        default=False, description="Whether the file is marked for deletion"
    )
