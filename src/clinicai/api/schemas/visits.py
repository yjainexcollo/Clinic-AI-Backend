...
class VisitListItemSchema(BaseModel):
    ...
    previous_status: Optional[str] = Field(None, description="Previous visit status (last distinct state)")
    status: str = Field(..., description="Current visit status")
    next_status: Optional[str] = Field(None, description="Best-effort prediction of the next visit status")
