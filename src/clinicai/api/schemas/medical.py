...
class TestOrderInfo(BaseModel):
    ...
    urgency: str = Field(
        default="routine", description="Urgency level (routine, urgent, stat)"
    )
    lab_location: Optional[str] = Field(None, description="Laboratory location")
    preparation_required: Optional[str] = Field(
...
class MedicalRecommendation(BaseModel):
    ...
    type: str = Field(
        ..., description="Type of recommendation (lifestyle, dietary, exercise, etc.)"
    )
...
class ActionPlanResponse(BaseModel):
    ...
    short_term_goals: List[str] = Field(
        default_factory=list, description="Short-term goals (1-2 weeks)"
    )
    long_term_goals: List[str] = Field(
        default_factory=list, description="Long-term goals (1-3 months)"
    )
    monitoring_plan: List[str] = Field(
        default_factory=list, description="Monitoring and follow-up plan"
    )
    patient_education: List[str] = Field(
        default_factory=list, description="Patient education topics"
    )
