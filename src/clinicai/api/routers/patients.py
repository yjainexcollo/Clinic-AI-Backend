    ...
    try:
        ...
        if content_type.startswith("application/json"):
            ...
            try:
                ...
                logger.warning(
                    "[AnswerIntake][JSON] Failed to decode patient_id '%s': %s; using raw value",
                    raw_pid,
                    e,
                )
            ...
            try:
                ...
                logger.warning(
                    "[AnswerIntake][FORM] Failed to decode patient_id '%s': %s; using raw value",
                    form_patient_id,
                    e,
                )
...
async def upload_medication_images(
    ...
    try:
        ...
        logger.info(
            "[WebhookImages] Incoming upload for patient_id=%s visit_id=%s",
            patient_id,
            visit_id,
        )
        ...
        for i, image in enumerate(file_list):
            try:
                logger.info(
                    "[WebhookImages] File[%d]: name=%s type=%s",
                    i,
        ...
        for i, image in enumerate(file_list):
            try:
                ...
                logger.info(
                    "[WebhookImages] Stored image[%d] id=%s filename=%s",
                    i,
                    str(getattr(inserted, "id", "")),
                    image.filename or f"image_{i+1}",
                )
...
async def get_intake_medication_image_content(request: Request, patient_id: str, visit_id: str, image_id: str):
    ...
    try:
        logger.info(
            f"[GetMedicationImage] Request: patient_id={patient_id[:50]}..., visit_id={visit_id}, image_id={image_id}"
        )
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(
                    ...
                    detail={
                        ...
                        "message": f"Invalid image ID format: {image_id}",
                    },
                )
            ...
            logger.error(
                f"[GetMedicationImage] Failed to get image document with ID {image_id}: {e}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=404,
        ...
        logger.info(
            f"[GetMedicationImage] Found image document: patient_id={doc.patient_id[:50] if len(str(doc.patient_id)) > 50 else doc.patient_id}..., visit_id={doc.visit_id}, blob_reference_id={doc.blob_reference_id}"
        )
        ...
        patient_id_matches = (
            doc.patient_id == str(internal_patient_id)
            or doc.patient_id == patient_encoded_id
            or doc.patient_id == patient_id  # Also check original request ID
        )

        logger.info(
            f"[GetMedicationImage] Patient ID match check: doc.patient_id={doc.patient_id[:50] if len(str(doc.patient_id)) > 50 else doc.patient_id}, internal={str(internal_patient_id)[:50]}..., encoded={patient_encoded_id[:50]}..., request={patient_id[:50]}..., matches={patient_id_matches}"
        )

        if not patient_id_matches or doc.visit_id != str(visit_id):
            logger.warning(
                f"[GetMedicationImage] Access denied: patient_id_match={patient_id_matches}, visit_match={doc.visit_id == str(visit_id)}"
            )
            raise HTTPException(
                status_code=403,
        ...
        try:
            ...
            if not blob_ref:
                ...
                raise HTTPException(
                    ...
                    detail={
                        "error": "NOT_FOUND",
                        "message": "Blob reference not found",
                    },
                )
            ...
            logger.error(
                f"[GetMedicationImage] Error fetching image from blob storage: {e}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=500,
...
async def get_intake_status(
    ...
    try:
        ...
        return fail(
            request,
            error="INTERNAL_ERROR",
            message="An unexpected error occurred while retrieving intake status",
        )
...
async def generate_pre_visit_summary(
    ...
    try:
        ...
        logger.error(
            f"PatientNotFoundError in generate_pre_visit_summary: patient_id={request.patient_id[:20] if request.patient_id else None}, error={e.message}"
        )
        return fail(
            http_request,
            error="PATIENT_NOT_FOUND",
        ...
        logger.error(
            f"VisitNotFoundError in generate_pre_visit_summary: visit_id={request.visit_id}, error={e.message}"
        )
        return fail(
            http_request,
            error="VISIT_NOT_FOUND",
...
async def get_pre_visit_summary(
    ...
    try:
            ...
            try:
                ...
                if not result.medication_images:
                    ...
                    if docs:
                        ...
                        logger.info(
                            f"[GetPreVisitSummary] Found {len(result.medication_images)} images in direct query"
                        )
        ...
        if images_meta:
            logger.info(
                f"[GetPreVisitSummary] Returning {len(images_meta)} medication images: {[img['filename'] for img in images_meta]}"
            )
...
@router.post(
    ...
    responses={
        ...
        422: {
            "model": ErrorResponse,
            "description": "Visit not completed or SOAP note not available",
        },
        500: {"model": ErrorResponse, "description": "Internal server error"},
    },
)
async def generate_post_visit_summary(
    ...
    try:
        ...
        logger.info(
            f"Generating post-visit summary for internal_patient_id: {internal_patient_id}, visit_id: {request.visit_id}, doctor_id: {doctor_id}"
        )
    ...
    try:
        ...
        if not data:
            raise HTTPException(
                status_code=404,
                detail={
                    "error": "POST_VISIT_SUMMARY_NOT_FOUND",
        ...
        if not visit.vitals:
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": "No vitals found for this visit",
                    "details": {},
                },
            )
...
async def list_patient_visits(
    ...
    try:
        ...
        return ok(
            request,
            data=VisitListResponse(visits=visit_items, total=len(visit_items)),
    ...
    try:
        ...
        if visit.transcription_session:
            transcription_session_data = TranscriptionSessionSchema(
                ...
                started_at=(
                    visit.transcription_session.started_at.isoformat()
                    if visit.transcription_session.started_at
                    else None
                ),
                completed_at=(
                    visit.transcription_session.completed_at.isoformat()
                    if visit.transcription_session.completed_at
                    else None
                ),
