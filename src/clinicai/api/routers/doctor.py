...
@router.get("/preferences", response_model=DoctorPreferencesResponse)
async def get_preferences():
    ...
    if not doc:
        return _defaults_for(doctor_id)
    return DoctorPreferencesResponse(
...
@router.post("/preferences", response_model=DoctorPreferencesResponse, status_code=status.HTTP_200_OK)
async def save_preferences(payload: UpsertPreferencesRequest):
