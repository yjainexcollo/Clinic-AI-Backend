...
async def transcribe_audio(
    ...
    # Log request (no PHI)
    logger.info(
        f"Transcribe audio request received: visit_id={visit_id}, language={language}, filename={audio_file.filename if audio_file else 'None'}"
    )
    ...
    print(
        f"🔵 Step 2 complete: content_type={content_type}, is_audio_like={is_audio_like}, is_mpeg_container={is_mpeg_container}, is_generic_stream={is_generic_stream}"
    )
    ...
    try:
        ...
        try:
            ...
            print(
                f"✅ Audio file saved to database: {audio_file_record.audio_id} (repo_call={repo_call_duration:.3f}s, total={blob_upload_duration:.3f}s)"
            )
            ...
            raise HTTPException(
                ...
                detail={
                    "error": "QUEUE_ERROR",
                    "message": f"Failed to queue transcription: {str(e)}",
                    "details": {},
                },
            )
        finally:
            ...
            if (
                ...
                and os.path.exists(normalized_file_path)
                and normalized_file_path != temp_file_path
            ):
...
async def get_transcription_status(
    ...
    try:
        ...
        if not visit.transcription_session:
            return ok(
                request,
                data={"status": "pending", "message": "Transcription not started"},
            )
        ...
        if transcription_status == "processing" and transcription_session.started_at:
            ...
            last_poll_age_minutes = (
                (now - transcription_session.last_poll_at).total_seconds() / 60
...
async def store_vitals(
    ...
    try:
        ...
        return {
            ...
            "vitals_id": f"{payload.visit_id}:vitals",
        }
    except PatientNotFoundError as e:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def get_vitals(
    ...
    try:
        ...
        if not visit.vitals:
            raise HTTPException(
                status_code=404,
                detail={"error": "VITALS_NOT_FOUND", "message": "No vitals found"},
            )
        return visit.vitals
    except HTTPException:
...
async def get_transcription_dialogue(
    request: Request,
    patient_id: str,
    visit_id: str,
    patient_repo: PatientRepositoryDep,
    ...
    try:
        ...
        transcription_status = (
            getattr(visit.transcription_session, "transcription_status", "").lower()
            if visit.transcription_session
            else "pending"
        )
        if (
        ...
        return ok(
            ...
            data=TranscriptionSessionDTO(
                ...
                structured_dialogue=getattr(
                    session, "structured_dialogue", None
                ),  # Return stored structured dialogue from database
            ),
            message="Success",
        )
...
async def structure_dialogue(
    ...
    try:
        ...
        logger.info(
            f"Structure dialogue result: {type(dialogue)}, length: {len(dialogue) if isinstance(dialogue, list) else 'N/A'}"
        )
