...
class DatabaseOptimizer:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        has_mobile_name_index = any(
            idx.get("name") == "mobile_name_compound" for idx in patients_indexes
        )
        ...
        health_status = {
            ...
            "visits": {"total": visits_count, "indexes": len(visits_indexes)},
        ...
        print(
            f"   Patients: {patients_count} total, {patients_with_visits} with embedded visits"
        )
        ...
        print(
            f"   Migration needed: {'Yes' if health_status['migration_needed'] else 'No'}"
        )
        print(
            f"   Optimization needed: {'Yes' if health_status['optimization_needed'] else 'No'}"
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive database setup and optimization"
    )
    parser.add_argument("--full-setup", action="store_true", help="Run complete setup")
    parser.add_argument(
        "--migrate-only", action="store_true", help="Run migration only"
    )
    parser.add_argument(
        "--optimize-only", action="store_true", help="Run optimization only"
    )
    parser.add_argument(
        "--health-check", action="store_true", help="Run health check only"
    )
    parser.add_argument(
        "--performance-test", action="store_true", help="Run performance tests only"
    )
