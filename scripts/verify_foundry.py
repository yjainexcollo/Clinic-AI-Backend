...
async def main():
    ...
    try:
        ...
        print("\n🔗 Next steps:")
        print(f"   - Azure AI Foundry → Monitoring → Requests → filter by Request ID `{response.id}` or deployment `{deployment}`")
        print(f"   - Log Analytics → Logs → run: AzureOpenAIRequests | where RequestId == '{response.id}'")
        print("   - Application Insights → Logs → query dependencies by operation_Id to cross-check")
